...
def render_product_management(
    ...
    # Importer: Modul-PDFs (Batch) – direkt in der Produktverwaltung sichtbar
    with st.expander(get_text_local("admin_module_pdf_import_expander_title_pm", "Modul-PDFs importieren (Batch)"), expanded=False):
        st.caption(get_text_local("admin_module_pdf_import_hint_pm", "Wähle eine oder mehrere Modul-PDFs. Alle erkannten Modulzeilen werden Produkten/Attributen zugeordnet."))
        sel_category_pm = st.selectbox("Ziel-Kategorie (falls in Datei nicht angegeben)", options=["Modul","Wechselrichter","Batteriespeicher","Wallbox","Zubehör"], index=0, key=f"module_pdf_category_select_prod{WIDGET_KEY_SUFFIX}")
        uploaded_pdfs_pm = st.file_uploader(
            get_text_local("admin_module_pdf_upload_label_pm", "Modul-PDF(s) auswählen"),
            type=["pdf"],
            accept_multiple_files=True,
            key=f"module_pdf_upload_prod{WIDGET_KEY_SUFFIX}"
        )
        col_imp_pma, col_imp_pmb = st.columns([1, 2])
        start_import_pm = col_imp_pma.button(get_text_local("admin_module_pdf_start_import_pm", "Import starten"), key=f"module_pdf_start_btn_prod{WIDGET_KEY_SUFFIX}")
        results_area_pm = col_imp_pmb.empty()

        if start_import_pm:
            try:
                if not uploaded_pdfs_pm:
                    st.warning(get_text_local("admin_module_pdf_no_files_pm", "Keine PDF-Dateien ausgewählt."))
                else:
                    import time
                    from pathlib import Path
                    safe_dir_pm = os.path.join(os.getcwd(), "data", "imports", "module_pdfs")
                    ts_dir_pm = time.strftime("%Y%m%d_%H%M%S")
                    target_dir_pm = os.path.join(safe_dir_pm, ts_dir_pm)
                    os.makedirs(target_dir_pm, exist_ok=True)

                    saved_files_pm = []
                    for up in uploaded_pdfs_pm:
                        orig = up.name or "modul.pdf"
                        safe_name = "".join(c if c.isalnum() or c in ('.','-','_') else '_' for c in orig)
                        pth = os.path.join(target_dir_pm, safe_name)
                        with open(pth, 'wb') as f:
                            f.write(up.getvalue())
                        saved_files_pm.append(pth)

                    try:
                        from tools.import_module_attributes_from_pdf import import_from_path as import_modules_batch_pm
                        summary_pm = import_modules_batch_pm(target_dir_pm, default_category=sel_category_pm)
                    except Exception as e_imp_pm:
                        st.error(get_text_local("admin_module_pdf_import_error_pm", f"Importer-Fehler: {e_imp_pm}"))
                        summary_pm = None

                    if summary_pm:
                        st.success(get_text_local("admin_module_pdf_import_done_pm", "Import abgeschlossen."))
                        st.write({
                            "pdf_dateien": len(saved_files_pm),
                            "gesamt_zeilen": summary_pm.get("total_rows"),
                            "produkte_angelegt_oder_gefunden": summary_pm.get("ensured_products"),
                            "produkte_aktualisiert": summary_pm.get("updated_products"),
                            "attribute_upserted": summary_pm.get("upserted_attributes"),
                            "ohne_modellname_übersprungen": summary_pm.get("skipped_no_model"),
                        })
                        details_pm = summary_pm.get("details") or []
                        max_show_pm = 5
                        if details_pm:
                            st.caption(get_text_local("admin_module_pdf_import_details_pm", "Details (erste 5):"))
                            st.json(details_pm[:max_show_pm])
                    else:
                        st.error(get_text_local("admin_module_pdf_import_no_summary_pm", "Kein Summary verfügbar."))
            except Exception as e_ui_imp_pm:
                st.error(get_text_local("admin_module_pdf_import_exception_pm", f"Fehler: {e_ui_imp_pm}"))
                st.text(traceback.format_exc())

    # Importer: Generisch (PDF/CSV/XLSX/JSON/YAML/TXT/MD/JPG/PNG)
    with st.expander("Dateien importieren (PDF/CSV/XLSX/JSON/YAML/TXT/MD/JPG/PNG)", expanded=False):
        st.caption("Importiert Modul-Produkte und deren Attribute aus Ordnern oder einzelnen Dateien. PDFs nutzen die spezialisierte Tabellenlogik; andere Formate werden generisch verarbeitet.")
        sel_category_any = st.selectbox("Ziel-Kategorie (falls in Datei nicht angegeben)", options=["Modul","Wechselrichter","Batteriespeicher","Wallbox","Zubehör"], index=0, key=f"generic_import_category_select_prod{WIDGET_KEY_SUFFIX}")
        generic_files_pm = st.file_uploader("Dateien hochladen", accept_multiple_files=True, type=["pdf","csv","xlsx","json","yml","yaml","txt","md","jpg","jpeg","png"], key=f"generic_import_upload_prod{WIDGET_KEY_SUFFIX}")
        run_generic_pm = st.button("Import starten (alle Formate)", key=f"generic_import_start_prod{WIDGET_KEY_SUFFIX}")
        if run_generic_pm and generic_files_pm:
            import time
            from pathlib import Path
            from tools.import_module_attributes_generic import import_any_from_path
            ...
            st.write({k: v for k, v in summary_any.items() if k != "details"})
            with st.expander("Details anzeigen"):
                details_any = summary_any.get("details", [])
                st.json(details_any)
                # Zusätzliche Darstellung je Datei (Mapping + Samples)
                for d in details_any:
                    if not isinstance(d, dict):
                        continue
                    if d.get("ok") and d.get("format") in ("csv","xlsx"):
                        st.markdown(f"**Datei:** {d.get('path')}  ")
                        headers = d.get("headers") or []
                        if headers:
                            st.caption("Gefundene Header:")
                            st.write(headers)
                        mapping = d.get("mapping") or []
                        if mapping:
                            st.caption("Header-Mapping:")
                            st.table(mapping)
                        samples = d.get("samples") or []
                        if samples:
                            st.caption("Beispiel (normalisierte Felder, max. 5 Zeilen):")
                            st.table(samples)
                        # XLSX: je Sheet anzeigen
                        if d.get("format") == "xlsx" and isinstance(d.get("sheets"), list):
                            st.caption("Arbeitsblätter:")
                            for s in d.get("sheets"):
                                st.markdown(f"- Blatt: {s.get('name')} | Zeilen: {s.get('total_rows')} | ensured: {s.get('ensured')} | updated: {s.get('updated')} | upserted: {s.get('upserted')}")
                                sh_map = s.get("mapping") or []
                                if sh_map:
                                    st.caption("  Mapping (Blatt):")
                                    st.table(sh_map)
                                sh_samples = s.get("samples") or []
                                if sh_samples:
                                    st.caption("  Samples (Blatt):")
                                    st.table(sh_samples)
    ...
    # Zusätzlicher, immer sichtbarer Attribut-Zugriff (unabhängig vom Bearbeitungsmodus)
    with st.expander(get_text_local("product_attributes_quick_access_expander_title", "Erweiterte Eigenschaften / Attribute (Direkt-Zugriff)"), expanded=False):
        # Prüfe Verfügbarkeit des Moduls
        if not pa_list_attributes or not pa_upsert_attribute or not pa_delete_attribute:
            st.warning(get_text_local("product_attributes_module_missing", "Attribut-Module nicht verfügbar."))
        else:
            # Produktauswahl
            all_products_for_attr = list_products_func(None) if callable(list_products_func) else []
            if not all_products_for_attr:
                st.info(get_text_local("product_attributes_no_products_info", "Keine Produkte vorhanden. Lege zuerst ein Produkt an."))
            else:
                # Anzeige-Label für Auswahl
                def _label_for_prod(p: Dict[str, Any]) -> str:
                    pid = p.get('id')
                    cat = p.get('category') or ''
                    brand = p.get('brand') or ''
                    model = p.get('model_name') or ''
                    return f"[{pid}] {cat} – {brand} {model}".strip()

                # Kategorien-Filter
                try:
                    category_options = list_product_categories_func() if callable(list_product_categories_func) else []
                except Exception:
                    category_options = []
                if not category_options:
                    # Fallback aus vorhandenen Produkten ableiten
                    category_options = sorted({(p.get('category') or '').strip() for p in all_products_for_attr if p.get('category')})
                all_label = get_text_local("product_attributes_filter_category_all", "Alle Kategorien")
                category_options_display = [all_label] + category_options
                selected_category = st.selectbox(
                    get_text_local("product_attributes_filter_category_label", "Kategorie filtern"),
                    options=category_options_display,
                    key=f"attr_quick_access_category{WIDGET_KEY_SUFFIX}"
                ) if category_options_display else all_label

                # Optionale Textsuche (Modell/Hersteller)
                search_text = st.text_input(
                    get_text_local("product_attributes_filter_search_label", "Suche (Modell/Hersteller)"),
                    key=f"attr_quick_access_search{WIDGET_KEY_SUFFIX}"
                )

                def _passes_filters(p: Dict[str, Any]) -> bool:
                    ok_cat = (selected_category == all_label) or ((p.get('category') or '') == selected_category)
                    if not ok_cat:
                        return False
                    q = (search_text or '').strip().lower()
                    if not q:
                        return True
                    return q in (p.get('model_name') or '').lower() or q in (p.get('brand') or '').lower()

                filtered_products = [p for p in all_products_for_attr if _passes_filters(p)]
                options_labels = [_label_for_prod(p) for p in filtered_products]
                options_map = { _label_for_prod(p): int(p.get('id')) for p in filtered_products if p.get('id') is not None }
                selected_label = st.selectbox(
                    get_text_local("product_attributes_select_product_label", "Produkt wählen"),
                    options=options_labels,
                    key=f"attr_quick_access_select{WIDGET_KEY_SUFFIX}"
                ) if options_labels else None

                if selected_label and selected_label in options_map:
                    selected_pid = options_map[selected_label]
                    # Hole Produktdetails für Kategorie
                    try:
                        selected_prod = get_product_by_id_func(selected_pid) if callable(get_product_by_id_func) else None
                    except Exception:
                        selected_prod = None
                    current_category_attr = (selected_prod.get('category') if isinstance(selected_prod, dict) else '') or ''

                    st.caption(get_text_local("product_attributes_selected_product_caption", "Ausgewähltes Produkt für Attribute"))

                    # Liste vorhandener Attribute
                    try:
                        attrs_list_q = pa_list_attributes(selected_pid) or []
                    except Exception:
                        attrs_list_q = []

                    if not attrs_list_q:
                        st.info(get_text_local("product_attributes_none_info", "Keine Attribute vorhanden. Du kannst unten welche hinzufügen."))
                    else:
                        st.markdown("**" + get_text_local("product_attributes_existing_header", "Vorhandene Attribute") + ":**")
                        for a in attrs_list_q:
                            cols_aq = st.columns([2.2, 3.2, 1.2, 0.8, 0.6])
                            cols_aq[0].caption(a.get('attribute_key') or "")
                            cols_aq[1].text(a.get('attribute_value') or "")
                            cols_aq[2].caption(a.get('unit') or "")
                            cols_aq[3].caption(str(a.get('display_order') or 0))
                            del_key_q = f"del_attr_quick_{selected_pid}_{a.get('id')}{WIDGET_KEY_SUFFIX}"
                            if cols_aq[4].button("", key=del_key_q, help=get_text_local("product_attributes_delete_help", "Attribut löschen")):
                                try:
                                    if pa_delete_attribute(int(a['id'])):
                                        st.success(get_text_local("product_attributes_deleted_success", "Attribut gelöscht."))
                                        st.session_state.selected_page_key_sui = "admin"
                                        st.rerun()
                                    else:
                                        st.error(get_text_local("product_attributes_deleted_fail", "Fehler beim Löschen."))
                                except Exception as e_del_q:
                                    st.error(get_text_local("product_attributes_deleted_exception", f"Fehler: {e_del_q}"))

                    st.markdown("---")
                    st.markdown("**" + get_text_local("product_attributes_add_update_header", "Attribut hinzufügen/aktualisieren") + ":**")

                    form_key_attr_q = f"attr_form_quick_{selected_pid}{WIDGET_KEY_SUFFIX}"
                    with st.form(key=form_key_attr_q, clear_on_submit=True):
                        a_key_q = st.text_input(get_text_local("product_attributes_key_label", "Key (z. B. cell_technology)"), key=f"{form_key_attr_q}_key")
                        a_val_q = st.text_input(get_text_local("product_attributes_value_label", "Wert"), key=f"{form_key_attr_q}_value")
                        cols_uoq = st.columns([1, 1])
                        a_unit_q = cols_uoq[0].text_input(get_text_local("product_attributes_unit_label", "Einheit (optional)"), key=f"{form_key_attr_q}_unit")
                        try:
                            a_order_default_q = 0
                        except Exception:
                            a_order_default_q = 0
                        a_order_q = cols_uoq[1].number_input(get_text_local("product_attributes_order_label", "Reihenfolge"), min_value=0, value=a_order_default_q, step=1, key=f"{form_key_attr_q}_order")
                        submit_attr_q = st.form_submit_button(get_text_local("product_attributes_upsert_button", "Speichern"))

                    if submit_attr_q:
                        if not a_key_q or not a_key_q.strip():
                            st.error(get_text_local("product_attributes_key_required", "Key ist erforderlich."))
                        else:
                            try:
                                saved_id_q = pa_upsert_attribute(selected_pid, current_category_attr, a_key_q.strip(), a_val_q.strip() if isinstance(a_val_q, str) else a_val_q, a_unit_q.strip() if isinstance(a_unit_q, str) else a_unit_q, int(a_order_q))
                                if saved_id_q:
                                    st.success(get_text_local("product_attributes_upsert_success", "Attribut gespeichert."))
                                    st.session_state.selected_page_key_sui = "admin"
                                    st.rerun()
                                else:
                                    st.error(get_text_local("product_attributes_upsert_fail", "Fehler beim Speichern des Attributs."))
                            except Exception as e_up_q:
                                st.error(get_text_local("product_attributes_upsert_exception", f"Fehler: {e_up_q}"))
    ...
    with st.expander(form_manual_title, expanded=(st.session_state.product_to_edit_id_manual is not None or not list_products_func(None))):
        ...
        # Nur anzeigen, wenn ein bestehendes Produkt bearbeitet wird (ID vorhanden)
        if st.session_state.product_to_edit_id_manual:
            st.markdown("---")
            with st.expander(get_text_local("product_attributes_expander_title", "Erweiterte Eigenschaften / Attribute"), expanded=False):
                if not pa_list_attributes or not pa_upsert_attribute or not pa_delete_attribute:
                    ...
                    current_pid_attr = int(st.session_state.product_to_edit_id_manual)
                    current_category_attr = product_data_for_manual_form.get('category') or ''
                    ...
                    if not attrs_list:
                        ...
                        for a in attrs_list:
                            ...
                            del_key = f"del_attr_{current_pid_attr}_{a.get('id')}{WIDGET_KEY_SUFFIX}"
                            if cols_a[4].button("", key=del_key, help=get_text_local("product_attributes_delete_help", "Attribut löschen")):
                                try:
                                    if pa_delete_attribute(int(a['id'])):
                                        st.success(get_text_local("product_attributes_deleted_success", "Attribut gelöscht."))
                                        st.session_state.selected_page_key_sui = "admin"
                                    ...
                                    st.error(get_text_local("product_attributes_deleted_exception", f"Fehler: {e_del}"))
                    ...
                    with st.form(key=form_key_attr, clear_on_submit=True):
                        a_key = st.text_input(get_text_local("product_attributes_key_label", "Key (z. B. cell_technology)"), key=f"{form_key_attr}_key")
                        a_val = st.text_input(get_text_local("product_attributes_value_label", "Wert"), key=f"{form_key_attr}_value")
                        cols_uo = st.columns([1, 1])
                        a_unit = cols_uo[0].text_input(get_text_local("product_attributes_unit_label", "Einheit (optional)"), key=f"{form_key_attr}_unit")
                        try:
                            a_order_default = 0
                        except Exception:
                            a_order_default = 0
                        a_order = cols_uo[1].number_input(get_text_local("product_attributes_order_label", "Reihenfolge"), min_value=0, value=a_order_default, step=1, key=f"{form_key_attr}_order")
                        submit_attr = st.form_submit_button(get_text_local("product_attributes_upsert_button", "Speichern"))
                    ...
                    if submit_attr:
                        if not a_key or not a_key.strip():
                            ...
                            try:
                                saved_id = pa_upsert_attribute(current_pid_attr, current_category_attr, a_key.strip(), a_val.strip() if isinstance(a_val, str) else a_val, a_unit.strip() if isinstance(a_unit, str) else a_unit, int(a_order))
                                if saved_id:
                                    st.success(get_text_local("product_attributes_upsert_success", "Attribut gespeichert."))
                                    st.session_state.selected_page_key_sui = "admin"
                                    st.rerun()
                                ...
                                st.error(get_text_local("product_attributes_upsert_exception", f"Fehler: {e_up}"))
