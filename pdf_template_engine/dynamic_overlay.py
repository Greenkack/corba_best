    ...
    for i in range(1, 7):
        ...
        for elem in elements:
            ...
            if i == 3 and text in {"46.296,00 €", "58.230,61 €"}:
                # Überspringen; später durch dynamischen Wert ersetzt
                continue

            # Seite 3: bestimmte numerische/legenden Texte immer schwarz erzwingen
            if i == 3:
                raw_txt = (draw_text or "").strip()
                force_black_patterns = [
                    "Ohne Strompreiserhöhung",
                    "Mit Strompreiserhöhung",
                ]
                # Numerische Achsenticks (z.B. 100.000 / 80.000 ... 0)
                import re as _re
                is_axis_tick = bool(_re.fullmatch(r"[0-9]{1,3}(\.[0-9]{3})*", raw_txt)) or raw_txt in {"0"}
                # Geldbeträge rechts (z.B. 153.082,14 € / 185.301,61 €) oder links (46.296,00 € ...)
                is_money = bool(_re.fullmatch(r"[0-9]{1,3}(\.[0-9]{3})*,[0-9]{2} \€", raw_txt))
                if raw_txt in force_black_patterns or is_axis_tick or is_money:
            ...
            if i == 3 and len(pos) == 4:
                try:
                    if pos[0] >= 350:
                        continue
                    if (text or "").strip() == "EUR" and pos[0] >= 100.0:
                        continue
                except Exception:
                    pass
            ...
            if is_footer_num:
                ...
                if key in center_keys and len(pos) == 4:
                    ...
                    c.drawString(draw_x, draw_y, str(draw_text))
        # Dynamische 10J-Kosten noch auf derselben Seite (vor showPage) zeichnen
...
def _draw_page3_right_chart_and_separator(c: canvas.Canvas, elements: List[Dict[str, Any]], dynamic_data: Dict[str, str], page_width: float, page_height: float) -> None:
    ...
    try:
        ...
        if len(legend_texts) < 2:
            ...
            legend_texts = ["ohne jährlicher Stromtariferhöhung", "mit jährlicher Stromtariferhöhung"]
    ...
    try:
        ...
        for img_b64, pos in images:
            ...
            try:
                ...
                c.restoreState()
        # Marken-Logos neben den Abschnittsüberschriften zeichnen (falls vorhanden)
        logos = [
            (dynamic_data.get("module_brand_logo_b64"), {"x": 200.0, "y_top": page_height - 205.0, "max_w": 60.0, "max_h": 24.0}),
            (dynamic_data.get("inverter_brand_logo_b64"), {"x": 200.0, "y_top": page_height - 395.0, "max_w": 60.0, "max_h": 24.0}),
            (dynamic_data.get("storage_brand_logo_b64"), {"x": 200.0, "y_top": page_height - 587.0, "max_w": 60.0, "max_h": 24.0}),
        ]
        for logo_b64, pos in logos:
            ...
            try:
                ...
                if iw <= 0 or ih <= 0:
                    continue
                max_w = float(pos.get("max_w", 60.0))
                max_h = float(pos.get("max_h", 24.0))
                x = float(pos.get("x", 200.0))
                y_top = float(pos.get("y_top", page_height - 205.0))
                scale = min(max_w / iw, max_h / ih)
                dw, dh = iw * scale, ih * scale
            except Exception:
                continue
...
def merge_with_background(overlay_bytes: bytes, bg_dir: Path) -> bytes:
    ...
    for page_num in range(1, 7):
        ...
        if base_page is not None:
            ...
            if page_num == 3:
                texts_to_remove = [
                    "Inflation Strompreise",
                    "Ohne Strompreiserhöhung", 
                    "Mit Strompreiserhöhung",
                    "ohne jährlicher Stromtariferhöhung",
                    "mit jährlicher Stromtariferhöhung"
                ]
