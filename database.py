...
def save_admin_setting(key: str, value: Any) -> bool:
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute("INSERT INTO admin_settings(key, value) VALUES(?, ?) ON CONFLICT(key) DO UPDATE SET value=excluded.value", (key, json.dumps(value) if not isinstance(value, str) else value))
        conn.commit()
        return True
    except Exception as e:
...
def get_brand_logo(brand_name: str) -> Optional[str]:
    """Liefert Base64-Logo für Marke aus admin_settings (key: brand_logo_<name>)."""
    if not brand_name:
        return None
    key = f"brand_logo_{brand_name.strip()}"
    ...
    if isinstance(logos, dict):
        v = logos.get(brand_name) or logos.get(brand_name.strip())
